function data = hdn_generaliseLagFEM(cfg,eeg1,eeg2)
...
for i = 1 : 2
    ...
    % downsample data
    fsample = 100;
    eeg = ft_resampledata(struct('resamplefs',fsample),eeg);
    time = eeg.time;
...
for j = 1 : numel(eegtmp)
    eegtmp{j} = rmfield(eegtmp{j},'comps_to_reject');
...
test_ts_r = nan(n_sigtypes,n_chans,n_lags);
test_ts_z = nan(n_sigtypes,n_chans,n_lags);
...
for sigtype = n_sigtypes:-1:1
    ...
    for lag = 1 : n_lags
        ...
        train_tSeries  = eeg_tSeries{1,sigtype};
        test_tSeries   = eeg_tSeries{2,sigtype};

        % offset data by lag
        for k = 1:numel(train_tSeries)
            train_lagSeries{k} = circshift(train_tSeries{k},lag_in_samples(lag),2); 
            train_lagSeries{k} = train_lagSeries{k}(:,ceil(n_lags/2):end-ceil(n_lags/2)); % chop ends
            test_lagSeries{k} = circshift(test_tSeries{k},lag_in_samples(lag),2); 
            test_lagSeries{k} = test_lagSeries{k}(:,ceil(n_lags/2):end-ceil(n_lags/2)); % chop ends
            train_lagDM{k} = DM{1}{k}(:,ceil(n_lags/2):end-ceil(n_lags/2)); % chop ends    
            test_lagDM{k} = DM{2}{k}(:,ceil(n_lags/2):end-ceil(n_lags/2)); % chop ends       
        end
        ...
        [test_ts_r(sigtype,:,lag),test_ts_z(sigtype,:,lag)] = hdn_model_test(train_weights,cat(2,test_lagSeries{:}),{cat(2,test_lagDM{:})});
    end
...
data = struct('label',{eeg_labels},'freq',[0.2 0.4 0.6 0.8 1],'time',lag_in_samples./fsample,...
              'r',permute(test_ts_r,[2 1 3]),...
              'z',permute(test_ts_z,[2 1 3]),...
