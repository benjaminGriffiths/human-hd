function [data,data_iem] = hdn_crossvalLagFEM(cfg,eeg)
...
for block = 1 : n_blocks
    ...
    for sigtype = numel(eeg_tSeries):-1:1
        ...
        for lag = 1 : n_lags
            ...
            test_lagSeries = circshift(test_tSeries,lag_in_samples(lag),2);
            ...
            [test_ts_r(sigtype,block,:,lag),test_ts_z(sigtype,block,:,lag)] = hdn_model_test(train_weights,test_lagSeries,test_lagDM);

            % tidy workspace
            clear train_tSeries test_tSeries train_weights lag
        end
    end
